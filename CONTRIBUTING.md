...
3. **Catch up npm** after fixing the secret:
   ...
   - **Option B:** If npm **never** received that version (the CI publish failed), check out the matching tag (e.g. `git checkout hirez-v1.0.15`), run `npm ci && npm run build`, then `node utils/pre-publish.js --version 1.0.15` and `npm run publish:local` as a maintainer with publish rights. If the version **already** exists on npm, use **Option A** or bump semver instead (npm does not allow overwriting an existing version).
